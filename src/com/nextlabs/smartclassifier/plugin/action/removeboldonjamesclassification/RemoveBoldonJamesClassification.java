package com.nextlabs.smartclassifier.plugin.action.removeboldonjamesclassification;
...
public class RemoveBoldonJamesClassification extends SharedFolderAction implements ExecuteOncePerFile {
	...
	private static final String TAGS_TO_BE_REMOVED = "Tags to be removed: ";
	...
	private static final String MESSAGE_FAIL = "Failed to remove Boldon James Classification";
	...
	private static final Logger logger = LogManager.getLogger(RemoveBoldonJamesClassification.class);
	private static final String ACTION_NAME = "REMOVE_BOLDON_JAMES_CLASSIFICATION";

	public RemoveBoldonJamesClassification() {
		super(ACTION_NAME);
	}
	...
	private boolean process(String path) {
		...
		Set<String> toBeRemovedSecurityCategoryTagSet = getParametersByKey(KEY_SPIF_SECURITY_CATEGORY_TAG_SET);
		logger.debug(SPIF_LOCATION + spifPath);
		logger.debug(TAGS_TO_BE_REMOVED + toBeRemovedSecurityCategoryTagSet.toString());

		if (!isSpifParametersValid(spifPath, toBeRemovedSecurityCategoryTagSet)) {
			// Assume Fail
			return false;
		}
		long start = System.nanoTime();
		List<String> spifList = loadSpifFromFile(spifPath, toBeRemovedSecurityCategoryTagSet);
		...
		String status = removeRecordClassification(powerClassifierManager, path, spifList);
	...
	private String removeRecordClassification(PowerClassifierManager powerClassifierManager, String path,
			List<String> spifList) {
		// Get Original XML
		String originalXml = powerClassifierManager.get(path);

		// Parse Original XML
		XmlManager xmlManager = new XmlManager();
		Map<String, Object> xmlMap = xmlManager.parse(originalXml);

		// Update XML (Remove SPIF)
		List<String> elements = ((List<String>) xmlMap.get(KEY_ELEMENTS));
		if (elements == null) {
			logger.info(LOG_GET_CLASSIFICATION_XML_OUTPUT + originalXml);
			return STATUS_XML_NOT_AVAILABLE;
		}
		elements.removeAll(spifList);
	...
	private List<String> loadSpifFromFile(String spifPath, Set<String> toBeRemovedSecurityCategoryTagSet) {
		SpifParser spifParser = new SpifParser();

		// Add lower case tags to list
		List<String> toBeRemovedSecurityCategoryTagSetList = new ArrayList<String>();
		for (String toBeRemovedSecurityCategoryTag : toBeRemovedSecurityCategoryTagSet) {
			toBeRemovedSecurityCategoryTagSetList.add(toBeRemovedSecurityCategoryTag.toLowerCase().trim());
		}

		// Load SPIF list from xml
		List<String> spifList = spifParser.parse(spifPath, toBeRemovedSecurityCategoryTagSetList);
		return spifList;
	}
}
